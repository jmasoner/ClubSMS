...
import com.clubsms.companion.CompanionService;
...
public class MainActivity extends AppCompatActivity {
    ...
    private Switch desktopModeSwitch;           // Toggle for desktop mode
    private TextView desktopStatusTextView;     // Desktop connection status
    ...
    private void initializeComponents() {
        ...
        // Desktop mode components
        desktopModeSwitch = findViewById(R.id.desktopModeSwitch);
        desktopStatusTextView = findViewById(R.id.desktopStatusTextView);
    ...
    /**
     * Initialize desktop mode toggle and status display
     */
    private void initializeDesktopMode() {
        if (desktopModeSwitch == null) {
            // UI elements not available yet - may need to add to layout
            return;
        }
        
        // Set initial state based on preference
        boolean isEnabled = CompanionService.isDesktopModeEnabled(this);
        desktopModeSwitch.setChecked(isEnabled);
        updateDesktopModeStatus(isEnabled);
    ...
    /**
     * Show confirmation dialog before enabling desktop mode
     */
    private void showDesktopModeConfirmation() {
        new android.app.AlertDialog.Builder(this)
            .setTitle(R.string.desktop_mode_dialog_title)
            .setMessage(R.string.desktop_mode_dialog_message)
            .setPositiveButton("Enable", (dialog, which) -> {
                enableDesktopMode();
            })
            .setNegativeButton("Cancel", (dialog, which) -> {
                // Reset toggle
                desktopModeSwitch.setChecked(false);
            })
            .setCancelable(false)
            .show();
    }
    ...
    /**
     * Enable desktop mode and start bridge service
     */
    private void enableDesktopMode() {
        CompanionService.enableDesktopMode(this);
        updateDesktopModeStatus(true);
        Toast.makeText(this, R.string.desktop_mode_enabled, Toast.LENGTH_LONG).show();
    }
    
    /**
     * Disable desktop mode and stop bridge service
     */
    private void disableDesktopMode() {
        CompanionService.disableDesktopMode(this);
        updateDesktopModeStatus(false);
        Toast.makeText(this, R.string.desktop_mode_disabled, Toast.LENGTH_SHORT).show();
    }
    
    /**
     * Update the desktop mode status display
     */
    private void updateDesktopModeStatus(boolean enabled) {
        if (desktopStatusTextView == null) return;
        ...
        if (enabled) {
            ...
            if (wm != null && wm.isWifiEnabled()) {
                ...
                desktopStatusTextView.setText("Desktop: ws://" + ipAddress + ":8765");
                desktopStatusTextView.setTextColor(getResources().getColor(R.color.green_success));
            } else {
                desktopStatusTextView.setText("Desktop: WiFi required");
                desktopStatusTextView.setTextColor(getResources().getColor(R.color.orange_warning));
            }
        } else {
            desktopStatusTextView.setText("Desktop: Off");
            desktopStatusTextView.setTextColor(getResources().getColor(R.color.text_secondary));
        }
    }
    ...
    protected void onResume() {
        super.onResume();
        // Refresh desktop mode status when returning to app
        if (desktopModeSwitch != null) {
            boolean isEnabled = CompanionService.isDesktopModeEnabled(this);
            desktopModeSwitch.setChecked(isEnabled);
            updateDesktopModeStatus(isEnabled);
