...
public class ClubContactManager {
    ...
    /**
     * Mark a member as opted out and return whether a matching member was found
     * Used by SmsReceiver for auto opt-out detection
     * 
     * @param phoneNumber The phone number to mark as opted out
     * @return true if the phone number was found in the system
     */
    public boolean markAsOptedOut(String phoneNumber) {
        if (phoneNumber == null || phoneNumber.trim().isEmpty()) {
            return false;
        }
        ...
        // Check if this number exists in our system
        boolean found = false;
        for (ClubMember member : allMembers) {
            String memberPhone = normalizePhoneNumber(member.getPhoneNumber());
            if (memberPhone.equals(normalized)) {
                member.setOptedOut(true);
                found = true;
                break;
            }
        }
        
        // Always add to opted-out set (even if not a member - in case they're a phone contact)
        optedOutNumbers.add(normalized);
        saveData();
        
        Log.i("ClubSMS", "Marked as opted out: " + phoneNumber + " (found=" + found + ")");
        return found;
    }
    
    /**
     * Normalize phone number for comparison
     * Removes non-digits and handles country code
     */
